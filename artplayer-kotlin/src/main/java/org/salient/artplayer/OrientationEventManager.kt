...
class OrientationEventManager {
    ...
    private var isRotate //0 代表方向锁定，1 代表没方向锁定 = 0
    ...
    fun orientationEnable(context: Context, orientationChangeListener: OnOrientationChangeListener?) {
     ...
     * 横屏(屏幕左边朝上)
     */
    private fun onOrientationLandscape() {
        val videoView: VideoView = MediaPlayerManager.Companion.instance().getCurrentVideoView()
        if (videoView != null) {
     ...
     * 反向横屏(屏幕右边朝上)
     */
    private fun onOrientationReverseLandscape() {
        val videoView: VideoView = MediaPlayerManager.Companion.instance().getCurrentVideoView()
        if (videoView != null) {
            if (currentOrientation == ActivityInfo.SCREEN_ORIENTATION_SENSOR_LANDSCAPE) return
            if (currentOrientation == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
     ...
     * 竖屏
     */
    private fun onOrientationPortrait() {
        val videoView: VideoView = MediaPlayerManager.Companion.instance().getCurrentVideoView()
        if (videoView != null) {
            if (currentOrientation == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
                return
